...
export const PHASE2_CONSOLIDATION_PROMPT: string;
export const MEMORY_RECALL_PROMPT: string;
export const MEMORY_RECALL_SUMMARY_TOKEN_LIMIT: number;
export const PHASE2_WORKSPACE_DIFF_FILE: string;
export const PHASE2_WORKSPACE_DIFF_MAX_BYTES: number;
export function phase1PiArgs(modelSpec?: string, systemPrompt?: string): string[];
export function phase1Prompt(transcript: string, rolloutPath: string, rolloutCwd: string): string;
export function buildMemoryRecallPrompt(basePath: string, memorySummary: string, maxTokens?: number): string;
export function stripMemoryCitations(text: string): { visibleText: string; citations: string[] };
export function parseMemoryCitation(citations: string[]): { entries: Array<{ path: string; lineStart: number; lineEnd: number; note: string }>; rolloutIds: string[] } | null;
export function extractMemoryCitationSessionIds(citations: string[]): string[];
export function stripAssistantMemoryCitations(message: any): { message: any; sessionIds: string[] };
export function recordMemoryCitationUsage(db: SqliteLike, sessionIds: string[], now?: number): number;
export function phase2Prompt(memoryRoot: string): string;
export function phase2PiArgs(modelSpec: string | undefined, prompt: string, providerExtension?: string): string[];
